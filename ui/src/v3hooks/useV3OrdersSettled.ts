...
export function useV3OrdersSettled() {
  ...
  function safeDecodeTrackingCode(value: string): string {
    try {
  ...
  const orderData = data?.orderSettleds.map((item) => {
    ...
    const trackingCode = safeDecodeTrackingCode(item.trackingCode || '');
...
const calculateV3OrderSettledPnl = (
  ...
  const orderSettledPnl = isLong
    ? marketPrice.sub(fillPrice).mul(size)
    : fillPrice.sub(marketPrice).mul(size);
