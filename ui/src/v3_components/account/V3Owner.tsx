...
export const V3Owner = () => {
  ...
  const { isFavorite, saveAccountFavorite, removeAccountFavorite } = useAccountFavorites(owner);
      ...
      <Flex mt={8} justifyContent="space-between" flexWrap="wrap">
        <Flex flexDirection="column">
          <Flex alignItems="center" flexWrap={{ base: 'wrap', md: 'nowrap' }}>
            ...
            <CopyButton variant="ghost" size="sm" color="white" iconSize="16px" value={owner} />
            <Link alignItems="center" href={basescanLink(owner)} target="_blank">
              <IconButton
          ...
          <Select mt={4} value={account?.id} onChange={handleAccountChange}>
            {data?.map((account) => (
