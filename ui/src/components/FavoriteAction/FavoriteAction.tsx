...
import {
  ...
  MenuList,
  IconButton,
  Tooltip, ModalOverlay, ModalContent, ModalHeader, ModalBody, Input, ModalFooter, Modal,
...
export const FavoriteAction = () => {
  ...
  return (
    <>
            ...
            {options && options.length > 0 ? options.map((account) => {
              ...
              return <MenuItem key={account}>
                <Flex width='100%' alignItems='center' justifyContent='space-between' gap={3}>
                  <Link to={`/${account}`}>
                    <AccountName address={account} fontSize='14px' _hover={{ textDecoration: 'underline' }} />
                  </Link>
                  <Flex alignItems='center'>
                    ...
                    >
                      <IconButton size='xs' _hover={{ background: 'none' }}
                                  _active={{ background: 'none' }} aria-label='edit-name' variant='ghost' color='white'
                                  icon={<EditIcon />} onClick={() => onRequestName(account)} />
                    </Tooltip>
                    <IconButton size='xs' _hover={{ background: 'none' }}
      ...
      {currentAddress && <Modal isOpen={isOpen} onClose={() => {
        setIsOpen(false);
        onReset();
      }}>
        <ModalOverlay />
        <ModalContent bg='navy.700' border='1px' borderColor='gray.900' borderRadius='5px' p={2}>
          ...
          <form
            onSubmit={(e: React.FormEvent<HTMLFormElement>) => {
              e.preventDefault();
              const accountName = (e.currentTarget.elements.namedItem('accountName') as HTMLInputElement)
                ?.value;
          ...
          >
            <ModalBody>
              <Flex flexDirection='column'>
                <Input defaultValue={currentName} placeholder='Input account name' type='text'
                       name='accountName' required />
              </Flex>
            </ModalBody>
            <ModalFooter gap={3}>
              {currentName &&
                <Button variant='outline' color='red.400' borderColor='red.400' type='button'
