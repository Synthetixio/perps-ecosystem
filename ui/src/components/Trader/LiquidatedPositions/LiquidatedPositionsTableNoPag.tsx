...
import { useTraderPositionClosedLiq } from '../../../hooks/useTraderPositionsClosedLiq';
...
export const LiquidatedPositionsTableNoPag = ({
  actionsRef,
  actionFilter,
  resetActionFilters,
  updateTradeId,
  ...
  const {
    processedClosedPositionData: liquidatedPositionData,
    traderClosedPositionQueryLoading: loading,
    traderClosedPositionQueryError: error,
  } = useTraderPositionClosedLiq({ isLiquidated: true });

  const noProcessedData = !liquidatedPositionData.length;
      ...
      >
        <>
          <Table bg="navy.700" __css={{ 'table-layout': 'fixed', width: 'full' }}>
            ...
            <Tbody>
              {loading && <LiquidatedPositionsLoading />}
