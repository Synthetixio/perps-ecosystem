...
import { TRADER_STATS } from '../queries/traderStats';
...
interface TraderTotalStatsInterface {
  totalPnl: Wei;
  totalVolume: Wei;
...
export const useTraderStats = () => {
  ...
  const traderTotalStats: TraderTotalStatsInterface = traderStats?.traders.reduce(
    (acc: TraderTotalStatsInterface, trader: Trader) => {
      return {
